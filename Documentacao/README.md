...
- Servidor de logs acessível em `<IP-SERVIDOR-LOGS>`

### Sensores disponíveis para teste

| Sensor     | IP               | sensor_id  | client_id |
|------------|------------------|------------|-----------|
| sensor-1   | <IP-SENSOR-1>   | sensor-1   | 20004     |
| sensor-2   | <IP-SENSOR-2>   | sensor-2   | 20004     |
| sensor-3   | <IP-SENSOR-3>   | sensor-3   | 20004     |
| sensor-04  | <IP-SENSOR-04>   | sensor-04  | 20004     |

### 2.1 Verificar o estado atual do sensor

```bash
ssh -p 12222 root@<IP-SENSOR-1>
...
# Para ajustar o timer de atualizacao dos sensores para testes em Laboratorio
Diretamente no sensor, via SSH:


ssh -p 12222 root@<IP-SENSOR-1>

# Editar o timer de teste
cat > /etc/systemd/system/vigilant-updater-test.timer << 'EOF'
[Unit]
Description=Vigilant Sensor Updater - Test Mode (every 10 minutes)

[Timer]
Unit=vigilant-updater.service
OnBootSec=1min
...
systemctl disable vigilant-updater.timer      # garante que o de produção está fora
systemctl enable  --now vigilant-updater-test.timer
...
Thu 2026-03-05 XX:XX:00 BRT  9min 47s Thu 2026-03-05 XX:XX:00 BRT  13s ago vigilant-updater-test.timer
Repita o mesmo nos outros sensores que quiser testar. Quando terminar os testes e quiser voltar ao timer semanal:


systemctl disable --now vigilant-updater-test.timer
systemctl enable  --now vigilant-updater.timer
